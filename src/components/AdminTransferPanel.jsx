...
export default function AdminTransferPanel({ tournament, onClose }) {
  const {
    transfers, players, isAdmin, claimedId,
    forceAcceptTransfer, adminCancelTransfer,
  } = useApp()
  ...
  const handleForceAccept = async (xfer) => {
    if (!confirm(`Force-accept this transfer? The spot moves to ${getPlayer(xfer.toPlayerId)?.name || 'the recipient'}.`)) return
    setBusyId(xfer.id)
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-end justify-center">
      <div className="bg-white rounded-t-3xl w-full max-w-md p-5 space-y-4 max-h-[90vh] flex flex-col overflow-y-auto">
        ...
        {pending.length === 0 && (
          <p className="text-sm text-gray-400 text-center py-6">No pending transfers for this event.</p>
        )}
        ...
        {pending.map(xfer => {
          ...
          return (
            <div key={xfer.id} className="border border-amber-200 bg-amber-50 rounded-xl p-3 space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 text-sm font-semibold text-gray-700 truncate">{toP?.name || '—'}</div>
              </div>
        ...
        <p className="text-[11px] text-gray-400 text-center pt-1">
          Force accept finalises the transfer immediately. The recipient's spot becomes registered, and the from-player's registration is cancelled.
        </p>
      </div>
    </div>
  )
