...
import {
  ...
  Plus, X, AlertCircle, CheckCircle, Users, ExternalLink, UserCog
} from 'lucide-react'
  ...
  return (
    <div className="space-y-4">
      ...
      {(() => {
        ...
        const hasInfo = isAdminAll
          ? (tournament.tikkieLink || tournament.totalPrice > 0)
          : (tournament.courts || []).some(c => c.tikkieLink || c.responsible)
        if (!hasInfo) return null
        return (
          <div className="card space-y-2">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wide">How to Pay</p>
            {isAdminAll ? (
              <>
                {tournament.totalPrice > 0 && (
                  <p className="text-sm text-gray-700">
                    Your share: <span className="font-bold text-lobster-teal">
                      €{(tournament.totalPrice / (tournament.maxPlayers || 1)).toFixed(2)}
                    </span>
                    <span className="text-xs text-gray-400"> (covers courts, food, drinks &amp; prizes)</span>
                  </p>
                )}
                {tournament.tikkieLink ? (
                  ...
                  <p className="text-xs text-gray-400">Pay via Playtomic — admin will confirm</p>
                )}
              </>
            ) : (
              <div className="space-y-2">
                <div className="flex items-center gap-1 mb-1">
                  ...
                  <span className="text-xs text-gray-500">Pay each court separately</span>
                </div>
                {(tournament.courts || []).map((c, i) =>
                  (c.responsible || c.tikkieLink || c.costPerPerson > 0) ? (
                    <div key={i} className="flex items-center justify-between gap-2 bg-gray-50 rounded-xl px-3 py-2">
                      <div>
                        ...
                        {c.responsible && <p className="text-xs text-gray-400">via {c.responsible}</p>}
                        {c.costPerPerson > 0 && <p className="text-xs text-lobster-teal font-semibold">€{c.costPerPerson}/pp</p>}
                      </div>
                      {c.tikkieLink && (
                        <a
                          href={c.tikkieLink}
