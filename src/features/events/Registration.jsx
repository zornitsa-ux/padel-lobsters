...
export default function Registration({ tournament, onNavigate }) {
  ...
  const pendingForTournament = transfers.filter(
    (t) => t.status === 'pending' && String(t.tournamentId) === String(tournament.id),
  )
  const pendingFromMe = pendingForTournament.find(
    (t) => claimedId && String(t.fromPlayerId) === String(claimedId),
  )
  const incomingForMe = pendingForTournament.filter(
    (t) => claimedId && String(t.toPlayerId) === String(claimedId),
  )
  ...
  return (
    <div className="space-y-4">
                        ...
                        <div className="bg-amber-50 border border-amber-200 rounded-xl px-3 py-2 space-y-2">
                          ...
                          {isOwner && recipient && (
                            <div className="flex gap-2">
                              <button
                                onClick={() =>
                                  setShareModal({ transferId: myPending.id, toPlayer: recipient })
                                }
          ...
          <div className="space-y-2">
            {waitlisted.map((reg, idx) => {
              ...
              return (
                <div
                  key={reg.id}
                  className="card flex items-center gap-3 border-l-4 border-orange-300"
                >
                  ...
                  {isAdmin && (
                    <div className="flex gap-1">
                      <button
                        onClick={() => handleMoveToRegistered(reg)}
      ...
      {(() => {
        ...
        const regs2 = getTournamentRegistrations(tournament.id).filter(
                  ...
                  <div>
                    ...
                    {isPlayerView && !claimedStr && (
                      <p className="text-sm text-gray-400 mb-3">
                        Sign in to see your own schedule.
                      </p>
                    )}
                    {roundNums.map((r) => {
          ...
          <div className="bg-white rounded-t-3xl w-full max-w-md p-6 space-y-5">
            ...
            {costPerPlayer > 0 && (
              <div className="bg-lobster-cream rounded-2xl px-4 py-3 flex items-center justify-between">
                ...
                <span className="text-2xl font-bold text-lobster-teal">
                  {fmtEur(costPerPlayer)}
                </span>
              </div>
            )}
