...
import STATIC_LOBSTER_WAY_CONTENT, { resolveLobsterWayContent } from '../../../data/lobsterWayContent'
...
export default function LobsterWayAdmin() {
  ...
  // stomp an in-progress edit (same guard Settings.jsx uses for the profile
  // form).
  ...
  const setCategory = (i, next) => update((prev) => replaceAt(prev, i, next))
  const moveCategory = (i, dir) => update((prev) => moveItem(prev, i, dir))
    ...
    return (
      <div className="-mx-4">
        ...
        <div className="px-4 pt-4">
          <SignInBanner
            role="admin"
            onNavigate={(page) => page === 'settings' && navigate('/account')}
          />
        </div>
      </div>
    )
