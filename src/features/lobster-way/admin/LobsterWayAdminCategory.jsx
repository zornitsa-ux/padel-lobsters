...
export default function LobsterWayAdminCategory({
  ...
  const setItem = (i, next) => set({ items: replaceAt(category.items, i, next) })
  const moveItemAt = (i, dir) => set({ items: moveItem(category.items, i, dir) })
  const deleteItemAt = (i) => set({ items: removeAt(category.items, i) })
