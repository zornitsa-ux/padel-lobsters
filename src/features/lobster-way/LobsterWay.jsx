...
import { resolveLobsterWayContent } from '../../data/lobsterWayContent'
        ...
        <ul className="space-y-1.5">
          {item.list.map((entry, i) => (
            <li key={i} className="text-sm text-gray-600 leading-relaxed flex gap-2">
              ...
              <span>
                {entry.label && <span className="font-semibold text-gray-700">{entry.label}: </span>}
                {entry.text}
              </span>
            </li>
          ))}
    ...
    <div className="-mx-4">
      ...
      <div className="px-4 pt-4 pb-8 space-y-5">
        {content.map((section) => (
          ...
          >
            {section.isStory ? (
              <details name={ACCORDION_GROUP} className="group bg-lob-cream border border-lob-teal/20 rounded-2xl px-4 py-3">
